...
class DiffuserModel(BaseModel):
    ...
    def init_output_process(self, config):
        ...
        if self.is_video:
            ...
            vae_scale_factor = 2 ** (
                len(self.vae_model.config.encoder_block_out_channels) - 1
            )
    ...
    def from_pretrained(cls, config, diffusers_config_args):
        ...
        """
        return cls(
            config.policy.diffusers_config, model_str=config.policy.model_name_or_path
        )
    ...
    def text_embedding(
        self, prompt_list: List[str], device="cuda", dtype=torch.bfloat16
    ):
        """
    ...
    def visual_embedding(self, input_image_list, device="cuda"):
        ...
        if self.is_video:
            with torch.no_grad():
                visual_embedding = self.vae_model.encode(
                    input_samples.to(torch.bfloat16), return_dict=False
                )[0]
            latents_mean = (
    ...
    def set_timestep_map(self):
        ...
        zero = torch.zeros(
            (1), dtype=self.timestep_map.dtype, device=self.timestep_map.device
        )
    ...
    def prepare_noise_latent(
        self, bsz, height, width, num_frames, dtype=torch.bfloat16, device="cuda"
    ):
        """
    ...
    def inference(
        ...
        # CFG
        prompt_embedding, prompt_attention_mask = self.text_embedding(prompt_list)
        negative_prompt_embedding, negative_prompt_attention_mask = self.text_embedding(
            [negative_prompt] * bsz
        )
        ...
        height, width = self.output_processer.classify_height_width_bin(
            height, width, ratios=self.ratio_bin
        )
        latents = self.prepare_noise_latent(
            bsz, height, width, frames, dtype=torch.float32
        )
        ...
        if self.is_video:
            ...
            visual_output = self.output_processer.resize_and_crop_tensor(
                visual_output, orig_width, orig_height
            )
            visual_output = self.output_processer.postprocess_video(
                visual_output, output_type="pil"
            )
        else:
            with torch.no_grad():
                visual_output = self.vae_model.decode(
                    latents / self.vae_model.config.scaling_factor, return_dict=False
                )[0]

            visual_output = self.output_processer.resize_and_crop_tensor(
                visual_output, orig_width, orig_height
            )
            visual_output = self.output_processer.postprocess(
                visual_output, output_type="pil"
            )
